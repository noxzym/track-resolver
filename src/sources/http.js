//source: https://github.com/AmandaDiscord/Volcano/blob/main/src/sources/http.ts
const icy = require("icy");
const tokenizer = require("@tokenizer/http");
const metadata = require("music-metadata");
const LimitedReadWriteStream = require('../util/LimitedReadWriteStream');
const { baseHTTPRequestHeaders } = require('../Constants'); 
const mimeRegex = /^(audio|video)\/(.+)$|^application\/(ogg)$/;

module.exports = async function HTTPSource(query) {
    const remote = new URL(query);
    let parsed;
	let headers;
    try {
		const toke = await tokenizer.makeTokenizer(query, { timeoutInSec: 10 }, { resolveUrl: true });
		headers = {
        ...
        const stream = await new Promise((res, rej) => {
			const req = icy.get({
				...
				headers: baseHTTPRequestHeaders
			}, response => {
	...
	if (headers["icy-description"]) extra.title = headers["icy-description"];
	if (headers["icy-name"]) extra.author = headers["icy-name"];
	return { parsed, extra };
}