...
module.exports = class trackResolver {
  ...
  async load(query) {
    ...
    if (isSoundcloudSearch || (url && url.hostname === soundCloudURL.hostname)) {
      ...
      if (data?.entries.length === 0 || !data) return { loadType: 'NO_MATCHES', ...payload };
      if (data.plData) {
        playlist = true;
      ...
      if (!data) return { loadType: 'NO_MATCHES', ...payload };
      ...
      const data = await YoutubeSource(resource || query, { isSearch: isYouTubeSearch || url && url.hostname.includes('youtu') ? false : !!(true || (!isYouTubeSearch && !isSoundcloudSearch)), loadAllTrack: this.options?.loadFullPlaylist }).catch((e) => Util.standardErrorHandler(e, payload));
      if (!data.entries || data?.entries.length === 0 || !data) return { loadType: 'NO_MATCHES', ...payload };
      const infos = data.entries.map((i) => ({
        identifier: i.id, title: i.title, author: i.channel.name, length: Math.round(i.duration * 1000), isStream: i.isLive ?? i.isLiveContent, isSeekable: i.isLive ? false : !i.isLiveContent, position: 0, uri: `https://youtube.com/watch?v=${i.id}`, sourceName: 'youtube',
      }));
      if (data.plData) {
        payload.playlistInfo = data.plData;
        playlist = true;
    ...
    return { loadType: payload.tracks.length > 1 && (isYouTubeSearch || isSoundcloudSearch || !isYouTubeSearch && !isSoundcloudSearch && !playlist) ? 'SEARCH_RESULT' : playlist ? 'PLAYLIST_LOADED' : 'TRACK_LOADED', ...payload };
  }
};
